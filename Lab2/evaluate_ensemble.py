...
from model_ensemble import EnsembleModel, create_ensemble
from dataset import PetNoseDataset, get_transforms


def evaluate_ensemble(snoutnet_path, alexnet_path, vgg16_path, 
                      batch_size=32, visualize=True, 
                      save_path="evaluation_samples/evaluation_samples_ensemble.png"):
    """
    Evaluate the ensemble model on the test dataset.
    
    Args:
        snoutnet_path: Path to SnoutNet weights
        alexnet_path: Path to AlexNet weights
        vgg16_path: Path to VGG16 weights
    ...
    num_params_snout = sum(p.numel() for p in model.snoutnet.parameters())
    num_params_alex = sum(p.numel() for p in model.alexnet.parameters())
    num_params_vgg = sum(p.numel() for p in model.vgg16.parameters())
    total_params = num_params_snout + num_params_alex + num_params_vgg
    
    print(f"\nTotal parameters across all models: {total_params:,}")
    print(f"  SnoutNet: {num_params_snout:,}")
    print(f"  AlexNet:  {num_params_alex:,}")
    print(f"  VGG16:    {num_params_vgg:,}")
    ...
    # Calculate MSE Loss
    mse_loss = torch.mean((all_predictions - all_ground_truth) ** 2).item()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluate ensemble model on test dataset')
    parser.add_argument('--suffix', type=str, default="",
                       help='Model suffix ("", "_aug_color", or "_aug_both")')
    parser.add_argument('-b', '--batch-size', type=int, default=32,
                       help='Batch size for evaluation (default: 32)')
    
    args = parser.parse_args()
    
    # Construct model paths based on suffix
    snoutnet_path = f"model_weights/snoutnet/best_snoutnet{args.suffix}.pth"
    alexnet_path = f"model_weights/alexnet/best_alexnet{args.suffix}.pth"
